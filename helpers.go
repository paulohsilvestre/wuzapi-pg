...
func callHookJson(myurl string, payload map[string]string, id int) {
	log.Info().Str("url", myurl).Msg("Sending POST to client " + strconv.Itoa(id))
	...
	_, err := clientHttp[id].R().SetHeader("Content-Type", "application/json").SetBody(payload).Post(myurl)
	if err != nil {
		log.Debug().Str("error", err.Error())
	}
}
